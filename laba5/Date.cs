...
namespace laba5
{
    public class Date
    {
        ...
        public static Date operator -(Date dateL, Date dateR)
        {
            
            Date date = new Date(dateL.Day, dateL.Month, dateL.Year);
            if (dateL >= dateR)
            {
                date.Year = dateL.Year - dateR.Year;
                if (date.Year > 0)
                {
                    date.Year--;
                    int month = (12 - dateR.Month) + dateL.Month;
                    if (month > 12)
                    {
                        month -= 12;
                        date.Year += 1;
                        ...
                        if (date.Month > 0)
                        {
                            int maxDays = dateR.MaxDaysInMonth();
                            int day = (maxDays - dateR.Day) + dateL.Day;
                            maxDays = date.MaxDaysInMonth();
                            if (day > maxDays)
                            {
                                day = day - maxDays;
                                date.Day = day;
                ...
                {
                    date.Month = dateL.Month - dateR.Month;
                    int maxDays = dateR.MaxDaysInMonth();
                    if (date.Month > 0)
                    {   
